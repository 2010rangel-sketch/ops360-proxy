...
app.get('/api/remocoes', async (req, res) => {
  try {
    ...
    for (const os of lista) {
      ...
      const cli   = cs?.cliente?.nome_razao || cs?.cliente?.nome_razaosocial || cs?.cliente?.nome_fantasia || cs?.cliente?.display || '—';
...
// ── Remoções: histórico mensal jan/2025 → mês atual ──────────────
let _remHistCache = null;
let _remHistFetchedAt = 0;
const REM_HIST_TTL = 2 * 60 * 60 * 1000; // 2h

async function buildRemocoesMensais() {
  const agora     = new Date();
...
app.get('/api/remocoes/historico', async (req, res) => {
  try {
    ...
    if (_remHistCache) return res.json({ ..._remHistCache, aviso: e.message });
    res.json({ ok: false, motivo: e.message });
  }
});
...
function buildVendasFromClientes(clientes, iniStr, fimStr) {
  ...
  for (const cli of clientes) {
    ...
    for (const s of servicos) {
      ...
      // Dedup: usa ID do serviço quando disponível (mais preciso); fallback por campos compostos
      const canceladoFlag = !!(s.data_cancelamento || (s.status_prefixo||'').includes('cancelado') || (s.status_prefixo||'').includes('rescindi'));
      const svcId = s.id || s.id_cliente_servico || s.id_produto || null;
      const chave = svcId
        ? `id:${svcId}|${canceladoFlag ? '1' : '0'}`
        : `${nome}|${s.nome || ''}|${s.data_venda || ''}|${s.endereco_instalacao?.logradouro || s.endereco_instalacao?.cidade || ''}|${canceladoFlag ? '1' : '0'}`;
...
app.get('/api/info-cancelamento', async (req, res) => {
  try {
    ...
    const mesIni = `${ano}-${String(mes).padStart(2,'0')}-01T03:00:00.000Z`; // 00:00 BRT do dia 1
    // Fim = 23:59:59 BRT do último dia = 02:59:59 UTC do 1º dia do próximo mês
    const _proxMes = new Date(ano, mes, 1); // 1º dia do próximo mês (aritmética local)
    const mesFim = `${_proxMes.getFullYear()}-${String(_proxMes.getMonth()+1).padStart(2,'0')}-01T02:59:59.999Z`;
    ...
    for (const a of listaIC) {
      ...
      const mfObj = a.motivo_fechamento_atendimento || a.motivo_fechamento;
      const motivo = !mfObj ? null
        : typeof mfObj === 'string' ? mfObj
        : Array.isArray(mfObj) ? mfObj.map(m => m?.descricao || m?.nome || '').filter(Boolean).join(', ') || null
        : (mfObj.descricao || mfObj.nome || mfObj.titulo || null);
